...
wizard.preview = function(){
	$button_id = "#wizard > article .MTLMCDONATECOM_CONTAINER > a";
	$button = $($button_id);
	$instance = $button.parent().attr('data-coinwidget-instance');
	$win = $("#MTLMCDONATECOM_WINDOW_"+$instance);
	...
	$win.find('> span.MTLMCDONATECOM_COUNT').html($counters.count+'<small>'+$lbl_count+'</small>');
	$win.find('> span.MTLMCDONATECOM_AMOUNT').html($counters.amount.toFixed(4)+'<small>'+$lbl_amount+'</small>');
	$counter_html = '';
	$right = 'auto';
	switch ($counter) {
		default:
	...
	if (MTLMCDonateCom.config[$instance]) {
		MTLMCDonateCom.config[$instance]['alignment'] = $alignment;
		$win.removeClass("MTLMCDONATECOM_WINDOW_BL MTLMCDONATECOM_WINDOW_BC MTLMCDONATECOM_WINDOW_BR MTLMCDONATECOM_WINDOW_AL MTLMCDONATECOM_WINDOW_AC MTLMCDONATECOM_WINDOW_AR");
		$win.addClass("MTLMCDONATECOM_WINDOW_"+$alignment.toUpperCase());
	}
	$button.parent().parent().css({'top':$top,'left':$left,'right':$right,'margin-left':$margin_left});
	
	$qrcode = ($qrcode == 'on')?true:false;
	if ($qrcode) {
		$win.find('.MTLMCDONATECOM_QRCODE').show();
	} else {
		$win.find('.MTLMCDONATECOM_QRCODE').hide();
	}
	...
	$button.find('img').attr('src',MTLMCDonateCom.source+'icon_'+$currency+'.png');
	$win.find('.COINWIDGET_INPUT_ICON').attr('src',MTLMCDonateCom.source+'icon_'+$currency+'.png');

	$wallet_val = $wallet;
	if (!$wallet) {
		switch ($currency) {
			default:
			case 'MetalMusicCoin': $wallet = MTLMCDonateCom.config[0].wallet_address; break;
		}
	} 
	if (MTLMCDonateCom.config[$instance])
		MTLMCDonateCom.config[$instance]['wallet_address'] = $wallet;
	$uri = $currency+':'+$wallet;
	$win.find('input').val($wallet);
	$win.find('.MTLMCDONATECOM_WALLETURI').attr('href',$uri);
...
wizard.code = {
	...
	, generate: function(options){
		$code = [];
		$code.push('<script src="http://mtlmcdonation.metalmusiccoin.pw/widget/coin.js"></script>');
		$code.push('<script>');
		$code.push('MTLMCDonateCom.go({');
	...
	, close: function(){
		$("#code").hide('drop',{direction:'down'},function(){
			$("#wizard").show('drop',{direction:'down'}, function(){
				$("#wizard .MTLMCDONATECOM_CONTAINER > a").click();
			});	
		});	
	}
	, close_all: function(){
...
wizard.start = function(){
	wizard.restart();
	$(".MTLMCDONATECOM_WINDOW").each(function(i,v){
		$instance = $(this).attr('id').replace('MTLMCDONATECOM_WINDOW_','');
		...
		$("#wizard").fadeIn(function(){
			$("#wizard div.loading").delay(500).hide('drop',{direction:'down'},500,function(){
				$("#wizard article").fadeIn(300, function(){
					$("#wizard article .MTLMCDONATECOM_CONTAINER a").click();
					wizard.preview();
				});
			});
		});
...
wizard.text = {
	defaults : {
		metalmusiccoin: {
			button: 	'Donate'
			, address: 	'My MetalMusicCoin Address:'
			, count:	'donations'
			, amount: 	'MTLMC'
		}
	}
